...
# Cleaned up and upgraded for Drupal 7 and 8 by Matthijs (2012-2016)
#
...
ModuleInfo = DefineModule(name='dumpDrupalSchema', author='Matthijs', version='1.3.0')
...
def generateFieldDefinition(column, comment_replace) :
  ...
  if type.endswith('int') :
    ...
    if column.scale > -1 and column.precision > -1 :
      ...
      specs.append("'precision' => 5")
      specs.append("'scale' => 2")
    ...
    ''' (Var)char '''
    if type == 'varchar' and column.characterSetName.find('ascii') > -1 :
      type = 'varchar_ascii'
  ...
  if not column.defaultValueIsNull and column.defaultValue :
    if column.defaultValue == 'NULL' and not column.isNotNull :
